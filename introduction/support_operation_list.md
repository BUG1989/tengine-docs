...
标 "?" 表示Tengine的serializer/include/tengine/v2/tm2_format.h中未定义，但在operator/operator有对应的cpp文件；

标 “+” 表示该算子在serializer里有定义，但没有注册；

存在大量Tengine算子一对多的情况，如Eltwise， Reduction，Comparison，Unary等；可能出现其他模型对应不上的情况（如TensorFlow和Caffe都有被Unary处理的算子，但它们互相之间对应不上）。

在ONNX一栏有一处标 "?" 的情况，是DepthwiseConvolution；原本是标了 "√" 但我在onnx的serializer里找不到对应；

在TF-Lite有“？？“表示《LESS GREATER》似乎应该是《LESS　EQUAL》？

在TensorFlow里有Unary中“？？”表示”tan“有错误，似乎是把tan打成了“tanh”（在tengine-module下tf_serializer中）。
